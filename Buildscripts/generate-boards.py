...
def parse_args():
    p = argparse.ArgumentParser(description="Generate project files from Buildscripts/boards.json")
    p.add_argument(
        "--list-changed",
        action="store_true",
...
def write_file_if_changed(path, content, list_changed=False):
    ...
    print("Wrote", path)
    if list_changed:
        print("\n--- Begin contents of changed file:", path, "---")
        # Print content safely; avoid very long binary blobs but this file is text
        print(content)
        print("--- End contents of changed file:", path, "---\n")
    return True


def write_kconfig(boards, list_changed=False):
    header = '''# THIS FILE IS AUTO-GENERATED by Buildscripts/generate-boards.py
...
def write_boards_h(boards, list_changed=False):
    header = '''// THIS FILE IS AUTO-GENERATED by Buildscripts/generate-boards.py
#pragma once
...
'''
    content = header + "\n".join(blocks) + footer
    return write_file_if_changed(OUT_BOARDS_H, content, list_changed=list_changed)


def write_board_cmake(boards, list_changed=False):
    header = '''# THIS FILE IS AUTO-GENERATED by Buildscripts/generate-boards.py
if (NOT WIN32)
    string(ASCII 27 Esc)
...
'''
    ...
    return write_file_if_changed(OUT_BOARDCMAKE, content, list_changed=list_changed)
...
def main():
    ...
    if not boards:
        ...
        return 0
    changed = False
    changed |= write_kconfig(boards, list_changed=args.list_changed)
    changed |= write_boards_h(boards, list_changed=args.list_changed)
    changed |= write_board_cmake(boards, list_changed=args.list_changed)
    ...
    # Always succeed (CI wrapper should detect git changes)
    return 0
