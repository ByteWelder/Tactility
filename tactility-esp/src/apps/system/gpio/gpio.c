...
//        if (tt_lvgl_lock(100) == TtStatusOk) {
...
static void app_show(App app, lv_obj_t* parent) {
    ...
    lv_obj_t* row_wrapper = lv_obj_create(wrapper);
    lv_obj_set_style_pad_all(row_wrapper, 0, 0);
    lv_obj_set_style_border_width(row_wrapper, 0, 0);
    lv_obj_set_size(row_wrapper, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
    ...
    for (int i = GPIO_NUM_0; i < GPIO_NUM_MAX; ++i) {
        ...
        if (column >= column_limit) {
            ...
            lv_obj_t* new_row_wrapper = lv_obj_create(wrapper);
            lv_obj_set_style_pad_all(new_row_wrapper, 0, 0);
            lv_obj_set_style_border_width(new_row_wrapper, 0, 0);
            lv_obj_set_size(new_row_wrapper, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
