...
static void on_navigate_up_pressed(lv_event_t* event) {
    auto* files_data = (FilesData*)lv_event_get_user_data(event);
...
static void on_file_pressed(lv_event_t* event) {
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        auto* files_data = static_cast<FilesData*>(lv_obj_get_user_data(button));
...
static void on_show(App app, lv_obj_t* parent) {
    auto* data = static_cast<FilesData*>(tt_app_get_data(app));

    lv_obj_set_flex_flow(parent, LV_FLEX_FLOW_COLUMN);

    lv_obj_t* toolbar = tt_toolbar_create(parent, "Files");
    tt_toolbar_set_nav_action(toolbar, LV_SYMBOL_CLOSE, &on_exit_app_pressed, nullptr);
