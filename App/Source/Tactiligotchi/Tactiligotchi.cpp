...
void TamagotchiApp::onShow(AppContext& /*context*/, lv_obj_t* parent) {
    ...
    auto* tb = toolbar_create(parent, *static_cast<AppContext*>(nullptr));
    ...
    // Create and start a 30s periodic Tactility timer
    update_timer = std::make_unique<tt::Timer>(
        tt::Timer::Type::Periodic,
        [this]() {
            // Defer all LVGL calls to the GUI thread
            lv_async_call(
                [](void* user_data) {
...
void TamagotchiApp::createStatBars(lv_obj_t* parent) {
    ...
    lv_obj_set_size(area, LV_PCT(90), 100);
    lv_obj_align(area, LV_ALIGN_CENTER, 0, 0);
...
void TamagotchiApp::createActionButtons(lv_obj_t* parent) {
    lv_obj_t* area = lv_obj_create(parent);
    lv_obj_set_size(area, LV_PCT(100), 80);
    lv_obj_align(area, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_clear_flag(area, LV_OBJ_FLAG_SCROLLABLE);

    struct Btn { lv_obj_t** b; const char* t; lv_event_cb_t cb; int x,y; };
    Btn btns[] = {
    ...
    for(auto& B:btns){
        *B.b = lv_btn_create(area);
        lv_obj_set_size(*B.b, 80, 30);
        lv_obj_align(*B.b, LV_ALIGN_CENTER, B.x, B.y);

        auto* lbl = lv_label_create(*B.b);
        lv_label_set_text(lbl, B.t);
        lv_obj_center(lbl);

        lv_obj_add_event_cb(*B.b, B.cb, LV_EVENT_CLICKED, this);
    }
}
...
void TamagotchiApp::updatePetDisplay() {
    ...
    switch(pet.mood){
      case PetMood::HAPPY:    c=PET_COLOR_HAPPY;    txt="😊 Happy!"; break;
      case PetMood::SAD:      c=PET_COLOR_SAD;      txt="😢 Sad..."; break;
      case PetMood::SICK:     c=PET_COLOR_SICK;     txt="🤒 Sick!"; break;
      case PetMood::SLEEPING: c=PET_COLOR_SLEEPING; txt="😴 Sleeping"; break;
      default:                c=PET_COLOR_NEUTRAL;  txt="😐 Okay"; break;
    }
    lv_obj_set_style_bg_color(pet_sprite, c, 0);
    if(pet.needs_cleaning){
        lv_label_set_text_fmt(status_label,"%s\n💩 Needs cleaning!",txt);
    } else {
        lv_label_set_text(status_label, txt);
    }
    int sz = 60 + static_cast<int>(pet.type)*10;
    lv_obj_set_size(pet_sprite, sz, sz);
}
...
void TamagotchiApp::animatePet(lv_color_t flash) {
    ...
    lv_async_call([](void* ud){
        static_cast<TamagotchiApp*>(ud)->updatePetDisplay();
    }, this);
}
