...
namespace tt::app {
    void Calibration::onShow(AppContext& context, lv_obj_t* parent) {
        TT_LOG_I("Calibration", "Starting calibration");
    ...
    void Calibration::onHide() {
        TT_LOG_I("Calibration", "Hiding calibration");
        if (label) {
            lv_obj_del(label);
            label = nullptr;
        }
    ...
    void Calibration::eventCallback(lv_event_t* e) {
        ...
        switch (app->step) {
            ...
            case 4: {
                ...
                TT_LOG_I("Calibration", "Calibration Results:");
                TT_LOG_I("Calibration", "Top-Left: x=%d, y=%d", app->rawX[0], app->rawY[0]);
                TT_LOG_I("Calibration", "Top-Right: x=%d, y=%d", app->rawX[1], app->rawY[1]);
                TT_LOG_I("Calibration", "Bottom-Left: x=%d, y=%d", app->rawX[2], app->rawY[2]);
                TT_LOG_I("Calibration", "Bottom-Right: x=%d, y=%d", app->rawX[3], app->rawY[3]);
                ...
                TT_LOG_I("Calibration", "X Range: %d to %d", minX, maxX);
                TT_LOG_I("Calibration", "Y Range: %d to %d", minY, maxY);
                TT_LOG_I("Calibration", "Suggested X: (ux - %d) * 239 / (%d - %d)", minX, maxX, minX);
                TT_LOG_I("Calibration", "Suggested Y: (uy - %d) * 319 / (%d - %d)", minY, maxY, minY);
                break;
            }
            default:
    ...
    void Calibration::logTouchData(uint16_t rawX, uint16_t rawY) {
        if (step < 4) {
            rawX[step] = rawX;
            rawY[step] = rawY;
            TT_LOG_I("Calibration", "Step %d: rawX=%d, rawY=%d", step, rawX, rawY);
        }
    }
