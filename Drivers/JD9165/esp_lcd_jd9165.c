...
static esp_err_t panel_jd9165_init(esp_lcd_panel_t *panel)
{
    ...
    // Note: Reading panel ID commented out - can cause timeout on ESP32-P4
    // uint8_t ID[3];
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        // Send command
        ESP_LOGD(TAG, "init cmd=0x%02X len=%d", init_cmds[i].cmd, (int)init_cmds[i].data_bytes);
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, init_cmds[i].cmd, init_cmds[i].data, init_cmds[i].data_bytes), TAG, "send command failed");
        // Small guard delay after every command to let the DSI DBI FIFO drain on ESP32-P4.
        vTaskDelay(pdMS_TO_TICKS(init_cmds[i].delay_ms ? init_cmds[i].delay_ms : 1));
    }
